...
TEST_F(ALockerRobotManager, should_throw_InvalidTicketException_when_take_bag_given_a_fake_S_size_ticket) {
...
TEST_F(ALockerRobotManager, should_throw_InvalidTicketException_when_take_bag_given_a_fake_M_size_ticket) {
    auto fake_ticket = Ticket(666, Size::kMedium);

    ASSERT_THROW(manager_->take(fake_ticket), Locker::InvalidTicketException);
}

TEST_F(ALockerRobotManager, should_take_correct_bag_when_take_bag_given_a_valid_L_size_ticket) {
    Bag bag{Size::kLarge};
    auto optional_ticket = manager_->store(&bag);
...
TEST_F(ALockerRobotManager, should_throw_InvalidTicketException_when_take_bag_given_a_fake_L_size_ticket) {
