...
## Tasking for LockerRobotManager

1.

Given：一个S号的包，S号的Locker有可用容量

When：通过LockerRobotManager存包

Then：存包成功，返回票据，包被存入S号的Locker里

2.

Given：一个S号的包，S号的Locker满了
...
Given：一个M号的包，PrimaryLockerRobot有可用容量

When：通过LockerRobotManager存包

Then：存包成功，返回票据，包被存入PrimaryLockerRobot

4.

Given：一个M号的包，PrimaryLockerRobot满了
...
Given：一个L号的包，SuperLockerRobot有可用容量

When：通过LockerRobotManager存包

Then：存包成功，返回票据，包被存入SuperLockerRobot

6.

Given：一个L号的包，SuperLockerRobot满了
